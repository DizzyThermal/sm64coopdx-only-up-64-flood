...
local function flood_define_level(bonus, level, name, goalPos, speed, area, type, customStartPos)
    levelIndex = if_then_else(game ~= GAME_ONLY_UP_64, level, area)
    gLevels[levelIndex] = { level = level, name = name, goalPos = goalPos, speed = speed, area = area, type = type, time = 0, customStartPos = customStartPos }
    table_insert(gMapRotation, levelIndex)
...
local function flood_load_only_up_64_levels()
    game = GAME_ONLY_UP_64
...
for mod in pairs(gActiveMods) do
    if gActiveMods[mod].incompatible ~= nil and gActiveMods[mod].incompatible:find("romhack") then
        ...
        elseif gActiveMods[mod].relativePath == "only-up-64" then
            flood_load_only_up_64_levels()
        else
            unsupported = true
