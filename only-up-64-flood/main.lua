...
hook_event(HOOK_UPDATE, function()
    ...
    -- Remove Star (From Last Area)
    local star_obj = obj_get_first_with_behavior_id(id_bhvStar)
    if star_obj ~= nil and
            gNetworkPlayers[0].currAreaSyncValid then
...
hook_event(HOOK_MARIO_UPDATE, function(m)
    ...
    if gPlayerSyncTable[0].finished then
        -- Enter Spectate Mode
        mario_set_full_health(m)
        if m.action ~= ACT_SPECTATOR and
                not players_all_dead() then
            m.area.camera.cutscene = 0
        ...
        local player_states = get_player_states()
        local alive_players = player_states.total - player_states.dead
        local last_alive = m.health > 0xFF and alive_players == 1
